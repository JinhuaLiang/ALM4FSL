...
class FSDKaggle18KFullsizeEvalution():
    def __init__(
        self, 
    ...
    def forward(self, verbose: bool = False):
        ...
        for cat, cnt in history.items():
            classwise_res[cat] = (torch.tensor(cnt, dtype=torch.float).mean() / 8).detach().numpy()  # todo: 
...
@hydra.main(version_base=None, config_path='../cfgs', config_name='fsdkaggle18k_fullsize.yaml')
def main(cfgs: OmegaConf) -> None:
    ...
    DataSet, Sampler, _ = prepare_data(data_source=db_name)
    ...
    eval_dataloader = DataLoader(eval_database, batch_size=cfgs['fewshot']['batch_size'], num_workers=4, pin_memory=True)
    prompt = 'this is a sound of '
    r""" Now begin our experiment."""
    fewshot = FSDKaggle18KFullsizeEvalution(
    ...
    overall_acc = list()
    for cat, cnt in _res.items():
        overall_acc.append(cnt)
    overall_acc = np.stack(overall_acc).sum() / len(eval_database)
